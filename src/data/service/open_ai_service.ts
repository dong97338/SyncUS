...
export default class OpenAIService {
  ...
  async getAnswer(userInput: string, userId: number, teamCode: number, date: Date): Promise<CodeResponse> {
    const openai = new OpenAI({apiKey: process.env.NEXT_PUBLIC_OPEN_AI_API_KEY, dangerouslyAllowBrowser: true});
    const chatPrompt = userInput
    try{
      const response = await openai.chat.completions.create({
        model: 'gpt-3.5-turbo',
        messages: [
          { role: 'system', content: `User ID: ${userId}, Team Code: ${teamCode}, Date: ${date}` },
